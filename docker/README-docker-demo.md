...
| Omniverse Kit App | **单独在宿主机跑**（`kit-app-template` 构建出的 `fii.houyiming_streaming.kit`）。监听 `:8011`(/ov + /kit/playback)、WebRTC `:12333/:12334`。**sim 前端现在直连 WebRTC，不再需要单独的 `:5183` 串流页** |
...
| Kit（宿主原生） | 8011 / 12333 / 12334 | — | 不在 compose 内；sim 前端直连 WebRTC（无需 5183 串流页） |

## 3. 验证 3D 串流（本机，用 P9）

1. 确认 **Kit App 已在宿主机跑起来**（`:8011` 控制 + `:12333/:12334` WebRTC 在监听）。sim 前端现在**直连 WebRTC**，不用再起 `:5183` 串流页。
