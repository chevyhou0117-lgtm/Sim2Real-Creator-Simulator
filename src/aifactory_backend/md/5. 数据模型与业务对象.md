...
| creator_binding_id | VARCHAR(100) | | AI Factory Creator 中的绑定对象 ID（用于3D联动）可以不使用                                       |
| created_at | DATETIME | ✓ | 创建时间                                                                            |
| updated_at | DATETIME | ✓ | 最后更新时间                                                                          |
...
| line_id | VARCHAR(36) | ✓ | 所属产线（外键→ProductionLine）, 1对多产线     |
...
| standard_changeover_time | DECIMAL(10,2) | | 标准换线时间（分钟）  错误x       |
...
| material_usage        | JSON(100)     | | 工序处理编码和使用比例（如 PCB、锡膏SAC305、阻容、IC/接口），用于仿真物料消耗逻辑                       |
...
方案的创建与编辑
新建方案：流程开始，创建一个新方案，其初始状态为 DRAFT。
编辑与删除：在 DRAFT 状态下，可以执行两个操作：
编辑配置：对方案进行配置，操作完成后状态回退至 DRAFT。
删除：仅创建者可以删除方案，删除后流程结束。

提交与就绪
提交就绪：在 DRAFT 状态下，提交方案进行完整性校验。校验通过后，方案状态变为 READY。
返回草稿：在 READY 状态下，可以返回 DRAFT 状态进行修改。

仿真运行
启动仿真：在 READY 状态下，可以启动仿真，方案状态变为 RUNNING。
取消运行：在 RUNNING 状态下，可以取消运行，结果不保存，方案状态返回 READY。
仿真完成：在 RUNNING 状态下，仿真完成后，方案状态变为 COMPLETED。
重新运行：在 COMPLETED 状态下，可以重新运行仿真，方案状态返回 RUNNING。

归档
归档：在 COMPLETED 状态下，可以将方案归档，方案状态变为 ARCHIVED。流程至此结束。
