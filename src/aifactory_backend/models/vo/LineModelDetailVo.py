...
class LineModelDetailVo(BaseModel):
    """线体模型详情响应 VO（驼峰命名）"""
    ...
    manufacturer: Optional[str] = Field(default=None, description="制造商，例如 Generic")
...
class LineAndEquipmentModelDetailVo(BaseModel):
    """线体挂载设备详情响应 VO（驼峰命名）"""
    ...
    name: Optional[str] = Field(default=None, description="节点名称（来自资产分类）")
    type: Optional[str] = Field(default=None, description="节点类型（来自资产分类）")
    instance_name: str = Field(..., description="设备实例名称")
    root_usd_path: str = Field(..., description="根USD文件路径")
    location_path: str = Field(..., description="位置路径")
    thumbnail_path: Optional[str] = Field(default=None, description="缩略图路径")
    instance_path: Optional[str] = Field(default=None, description="Instance 路径（后续实现设备组成产线）")
    prim_path: Optional[str] = Field(default=None, description="设备对应的 prim_path")
