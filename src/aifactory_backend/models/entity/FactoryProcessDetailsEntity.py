...
class FactoryProcessDetails(Base):
    """制程详情 ORM 实体（v2）- ref_id 从节点表迁移至此"""
    __tablename__ = "factory_process_details"
    ...
    ref_id = Column(String(36), nullable=True, comment="关联 base_stage.stage_id，获取制程基础信息")

    total_capacity = Column(Integer, nullable=True, comment="制程总产能（pcs/day），实例层计算值")
    ...
    def __repr__(self):
        return f"<FactoryProcessDetails(id={self.id}, factory_asset_id={self.factory_asset_id}, ref_id={self.ref_id})>"
