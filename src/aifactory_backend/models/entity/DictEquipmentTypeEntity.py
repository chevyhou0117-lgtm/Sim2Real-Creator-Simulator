...
class DictEquipmentType(Base):
    """设备类型字典 ORM 实体"""
    __tablename__ = "dict_equipment_type"

    equipment_type_id = Column(String(36), primary_key=True, default=generate_snowflake_id, comment="主键ID")
    equipment_type_code = Column(String(50), nullable=False, unique=True, comment="类型编码")
    equipment_type_name = Column(String(200), nullable=False, comment="类型名称")
    ...
    def __repr__(self):
        return f"<DictEquipmentType(equipment_type_id={self.equipment_type_id}, code={self.equipment_type_code})>"
