...
class LineModelDetailCreateDto(BaseModel):
    """创建线体模型详情请求 DTO"""
    ...
    manufacturer: Optional[str] = Field(default=None, description="制造商，例如 Generic", max_length=100)
...
class LineModelDetailUpdateDto(BaseModel):
    """更新线体模型详情请求 DTO（id 必填）"""

    model_config = ConfigDict(alias_generator=to_camel, populate_by_name=True)

    id: SnowflakeIdIn = Field(..., description="线体模型详情主键ID（雪花算法，18~19位整数）")
...
class LineModelDetailDeleteDto(BaseModel):
    """删除线体模型详情请求 DTO"""

    id: SnowflakeIdIn = Field(..., description="线体模型详情主键ID（雪花算法，18~19位整数）")


class LineModelDetailQueryDto(PageRequest):
    """分页查询线体模型详情请求 DTO"""
...
class LineAndEquipmentRelDto(BaseModel):
    """查询某线体挂载了哪些设备的请求 DTO"""

    id: SnowflakeIdIn = Field(..., description="线体模型详情主键ID")
